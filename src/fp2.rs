...
const NonResidue2Power1: Fp = Fp(blst_fp {
    l: [
...
const NonResidue2Power2: Fp = Fp(blst_fp {
    l: [
        3526659474838938856,
...
const NonResidue2Power4: Fp = Fp(blst_fp {
    l: [
...
const NonResidue2Power3: Fp = Fp(blst_fp {
    l: [
        4897101644811774638,
        3654671041462534141,
        569769440802610537,
...
const NonResidue2Power5: Fp = Fp(blst_fp {
    l: [
        9875771541238924739,
        3094855109658912213,
        5802897354862067244,
...
impl Fp2 {
    ...
    pub fn conjugate(&mut self, x: &Self) {
        let c0 = Fp(x.0.fp[0]);
        let c1 = -x.c1();
        *self = Self::new(c0, c1);
    }

    pub fn mul_by_nonresidue_1p1_self(&mut self) {
        ...
        let non_residue_1p1: Fp2 = Fp2::new(
            Fp(blst_fp {
                l: [
                    506819140503852133,
    ...
    pub fn mul_by_nonresidue_1p2_self(&mut self) {
        ...
        let non_residue_1p2: Fp2 = Fp2::new(
            Fp::zero(),
            Fp(blst_fp {
                l: [
                    14772873186050699377,
        ...
        self.mul_assign(&non_residue_1p2);
    }

    pub fn mul_by_nonresidue_1p3_self(&mut self) {
    ...
    pub fn mul_by_nonresidue_1p4_self(&mut self) {
        // 4002409555221667392624310435006688643935503118305586438271171395842971157480381377015405980053539358417135540939437
        let non_residue_1p4: Fp = Fp(blst_fp {
            l: [
                9875771541238924739,
                3094855109658912213,
                5802897354862067244,
        ...
        unsafe {
            blst_fp_mul(&mut self.0.fp[0], &self.0.fp[0], &non_residue_1p4.0);
            blst_fp_mul(&mut self.0.fp[1], &self.0.fp[1], &non_residue_1p4.0);
        };
    }

    pub fn mul_by_nonresidue_1p5_self(&mut self) {
    ...
    fn mul_by_fp(&mut self, x: &Self, f: &Fp) {
        unsafe {
            blst_fp_mul(&mut self.0.fp[0], &x.0.fp[0], &f.0);
            blst_fp_mul(&mut self.0.fp[1], &x.0.fp[1], &f.0);
        };
    }

    pub fn mul_by_nonresidue_2p1(&mut self, x: &Self) {
        self.mul_by_fp(&x, &NonResidue2Power1);
    }
    pub fn mul_by_nonresidue_2p2(&mut self, x: &Self) {
        self.mul_by_fp(&x, &NonResidue2Power2);
    }
    pub fn mul_by_nonresidue_2p3(&mut self, x: &Self) {
        self.mul_by_fp(&x, &NonResidue2Power3);
    }
    pub fn mul_by_nonresidue_2p4(&mut self, x: &Self) {
        self.mul_by_fp(&x, &NonResidue2Power4);
    }
    pub fn mul_by_nonresidue_2p5(&mut self, x: &Self) {
        self.mul_by_fp(&x, &NonResidue2Power5);
    }
